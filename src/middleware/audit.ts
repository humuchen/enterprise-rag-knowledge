...
import { principalAuditId, type Principal } from '../principal';
...
export async function auditLogger(
  ...
  // 身份取自服务端解析的可信主体，不再信任客户端自报的 x-user-id（可被伪造）。
  const principal = (request as { principal?: Principal }).principal;
  const userId = principal
    ? principalAuditId(principal, request.headers['x-api-key'] as string | undefined)
    : 'anonymous';
